...
public class GrabSZStockCodeThread implements Callable<List<Map<String,Object>>>{
	private int s;
	private int e;
	public GrabSZStockCodeThread(int s,int e){
		this.s = s;
		this.e = e;
	}
    	...
    	for(int i=s;i<=e;i++){
    		if(GrabQutoesBean.start){
				...
				if(!name.equals("\"\"")&&!name.equals("")){//StringUtils.isNotEmpty(name)
					...
					tmp.put("name", name);
					int count = dao.query(key.substring(2, key.length()), key.substring(0,2));
					if(count<=0){
	...
	public List<Map<String,Object>> call() throws Exception {
		Thread.currentThread().setName("szstock["+s+"-"+e+"]");
		return run();
	}
}
